...
class SecureFolder:
	...
	def abrir(self):
		...
		self.nombre_carpeta_abrir = Entry(frame_abrir)
		self.nombre_carpeta_abrir.focus()
		self.nombre_carpeta_abrir.grid(row = 1, column = 1)
	...
	def abrir_carpeta(self):
		...
		os.system("REN " +  self.nombre_carpeta_abrir.get() + ".{2559a1f2-21d7-11d4-bdaf-00c04f60b9f0}" + " " + self.nombre_carpeta_abrir.get())
		delay
		wb.open_new(self.nombre_carpeta_abrir.get())
		try:
			self.abrir.destroy()
	...
	def cerrar(self):
		self.cerrar = Toplevel()
		self.cerrar.iconbitmap("icono2.ico")
		frame_cerrar = LabelFrame(self.cerrar)
		frame_cerrar.grid(row = 0, column = 0, columnspan = 3, pady = 20)

		Label(frame_cerrar, text = "Cerrar carpeta abierta").grid(row = 1, column = 0)
		
		ttk.Button(frame_cerrar, text = "Cerrar", command = self.cerrar_carpeta).grid(row = 1, column = 3)

	def cerrar_carpeta(self):

		os.system("REN " + self.nombre_carpeta_abrir.get() + " " + self.nombre_carpeta_abrir.get() + ".{2559a1f2-21d7-11d4-bdaf-00c04f60b9f0}")
